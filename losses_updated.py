...
def sigmoid_focal_loss(inputs, targets,iou_scores_cls, num_boxes, alpha: float = 0.25, gamma: float = 5):
    ...
    iou_scores, _ = iou_scores_cls.max(dim=-1)
    # Identify false negatives: targets indicate a foreground class, but predictions are closer to background
    is_background_pred = (prob.argmax(dim=-1) == 0)  # Check if the predicted class is background
    is_foreground_target = (targets.argmax(dim=-1) != 0)  # Check if the target is a foreground class
    #print(f'is_background_pred:{is_background_pred.shape}')
    #print(f'is_foreground_target:{is_foreground_target.shape}')
    # False negatives: model predicted background but IoU with a target is above the threshold
    false_negative_condition = is_background_pred & is_foreground_target & (iou_scores > iou_threshold)
    #print(f'false_negative_condition:{false_negative_condition.shape}')

    # Create IoU weights: apply IoU penalties for false negatives
    iou_weights = torch.ones_like(iou_scores)
    #print(f'iou_weights:{iou_weights.shape}')

    iou_weights = torch.where(false_negative_condition, iou_scores, iou_weights)  # Penalize false negatives

    # Apply IoU weights to the loss (expand to match shape of loss)
    loss = loss * iou_weights.unsqueeze(-1)  # unsqueeze to match shape of loss
