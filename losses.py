...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if self.ia_bce_loss:
            ...
            loss_ce = position_supervised_loss(src_logits, norm_cls_iou_func_targets, num_boxes, alpha=self.focal_alpha, gamma=2.0) * src_logits.shape[1]
            ...
            loss_ce = sigmoid_varifocal_loss(src_logits, cls_iou_targets, num_boxes, alpha=self.focal_alpha, gamma=2.0) * src_logits.shape[1]
        else:
            ...
            loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=2.0) * src_logits.shape[1]
...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: torch.Tensor = None, gamma: float = 2):
    """
    ...
    Args:
        ...
        alpha: (optional) A tensor of weighting factors in range (0,1) to balance
                positive vs negative examples for each class. If None, no weighting is applied.
    ...
    if alpha is not None:
        # Ensure alpha is of the same shape as targets
